...
export const unverifyPresident = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!president.isVerified) {
      return res.status(400).json({ message: 'President is not verified' });
    }
...
// Unverify Cabinet
export const unverifyCabinet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { cabinetId } = req.body;
    const techHeadId = req.user?.id;
    ...
    if (!cabinetMember.isVerified) {
      return res.status(400).json({ message: 'Cabinet member is not verified' });
    }
...
// Unverify Member
export const unverifyMember = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { memberId } = req.body;
    const techHeadId = req.user?.id;
